...
-- v3 (current)
select e.route_id, r.name, count() as amount, avg(e.effective_distance) as avg_distance, a.best_pace
from exercises as e
...
-- v2
select e.route_id, r.name, count() as amount, avg(e.effective_distance) as avg_distance,
...
from exercises as e inner join routes as r on e.route_id = r._id inner join
	(select route_id, count() as antal from exercises group by route_id having count() > 1) as a on e.route_id = a.route_id inner join
